...
function love.update(dt)
    ...
    paddle2:update(dt * 0.5)
    if ball.x > VIRTUAL_WIDTH * INSTINCT then
        if ball.y < paddle2.y + paddle2.height *0.35 then
            paddle2.dy = -PADDLE_SPEED
        elseif ball.y > paddle2.y + paddle2.height * 0.65 then
            paddle2.dy = PADDLE_SPEED
        else
            paddle2.dy = 0
        end
